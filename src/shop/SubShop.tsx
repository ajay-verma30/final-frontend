  ...
  useEffect(() => {
    const loadProducts = async () => {
      try {
        ...
        // Read URL params and resolve slugs → IDs after products load
        const catSlug = searchParams.get("category");
        const subSlug = searchParams.get("subcategory");

        if (catSlug) {
          const matched = data.find(
            (p) => p.category_name?.toLowerCase().replace(/\s+/g, '-') === catSlug
          );
    ...
    loadProducts();
  }, []); // run once on mount — URL params read after data loads
