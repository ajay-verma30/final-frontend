...
import { Trash2, Plus, Loader2, X, FolderOpen,Image as ImageIcon } from "lucide-react";
...
const Categories: React.FC = () => {
  ...
  const fetchCategories = async () => {
    ...
    try {
      ...
      // Backend response structure check: res.data.data
  ...
  try {
    ...
    const categoryId = res.data.categoryId;
      ...
      if (imageFile) {
        ...
        await api.post("/api/categories/upload-size-chart", formData, {
 ...
 const resetForm = () => {
  ...
  return (
    <div className="flex h-screen w-screen bg-gradient-to-br from-slate-50 via-slate-50 to-slate-100">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {categories.map((cat) => (
                        <tr key={cat.id} className="hover:bg-slate-50/50 transition-colors group">
                          ...
                          {/* 2. New Assets Column: Size Chart Preview */}
                          <td className="px-6 py-4">
      ...
      {/* CREATE MODAL (Wahi rahega jo tumne bheja tha) */}
       {showModal && (
